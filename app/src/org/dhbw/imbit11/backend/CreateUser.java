...
@WebServlet({"/CreateUser"})
 public class CreateUser extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
	 ...
	 *  student user; this method expects the groupid to be encoded ( first two digits: checksum; remaining digits: groupID*23)
	 *  If the registration was successful the user is redirected to a landing page; if not, the form is called again and
...
//		//System.out.println(role);
		...
		switch(role){
		case "student": url = "/Registration?g="+groupnumber; break;
		//case "admin": url = createNewAdmin(request, realm, email, lastname, firstname, encryptedPassword, gender); break;
		case "professor": url = "/Admin" ; break;
...
//		//System.out.println("Der User " + firstname +" " + lastname + " wird registriert. Gender: " + gender + " "+genderValue );
				
		String password = request.getParameter("password"); 
		String password_repeat = request.getParameter("password_repeat"); 
		
		
		//setting attributes so that the user has not to reenter the data if registration fails
		...
		if(email != null && !email.equals("")  && firstname != null   && !firstname.equals("") && lastname != null   && !lastname.equals("")  && password != null  && !password.equals("") ){
			...
			if(password.equals(password_repeat)){
				...
				if(gender!=-1){
					...
					if(!realm.userExists(email)){
						...
						switch(role){
							case "student": url = createNewStudent(request, realm, email, lastname, firstname, encryptedPassword, gender); break;
							//case "admin": url = createNewAdmin(request, realm, email, lastname, firstname, encryptedPassword, gender); break;
							case "professor": url = createNewProfessor(request, realm, email, lastname, firstname, encryptedPassword, gender); break;
			...
			request.setAttribute("status", "You have to fill in every field before submitting.");
		}
	...
	protected String createNewStudent(HttpServletRequest request, UserRealm realm, String email, String lastname, String firstname, String encryptedPassword, int gender){
		String groupnumber = request.getParameter("groupnumber"); 
		...
		if (!resolveGroupid(groupnumber).equals("false")){
			...
			if(realm.groupExists(groupnumber)){
						...
						realm.createNewUser(unverifiedEmail, lastname, firstname, encryptedPassword, "student", groupnumber, gender);
	...
	protected void sendConfirmationMail(String email, String firstname, String lastname, String unverifiedEmail, HttpServletRequest request){
        ...
        try {
            ...
            mailclient.sendMail(email, "Please confirm your registration!", msgBody);
        } catch(Exception e){
        	e.printStackTrace();
	...
	/*
	 * This method resolves the groupID entered ( first two digits: checksum of remaining digits; remaining digits: groupID*23)
	 */
